...
if __name__ == '__main__':
    ...
    layer_names = {
        "conv2": "features.13",  # First MaxPool
        "conv3": "features.23",  # Second MaxPool
        "conv4": "features.33",  # Third MaxPool
        "conv5": "features.43",  # Fourth MaxPool
        "combi": ["features.13", "features.23", "features.33", "features.43"],  # Combining 4 layers in one pass
    }
    ...
    for i, layer_name in enumerate(layer_names.keys()):
        ...
        plt.savefig(f"grid_vgg_{target_index}_{layer_name}.png", format="png", pad_inches=0.0)
